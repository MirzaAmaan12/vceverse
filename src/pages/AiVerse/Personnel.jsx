...
import { thanveerImg, salimImg, swethaImg, prakashImg } from "../shared/foundationImages";
...
function ProfileCard({ person }) {
  return (
    <motion.div 
      whileTap={{ scale: 0.98 }} 
      className="relative p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-white/[0.02] border border-white/10 flex flex-col items-center text-center overflow-hidden hover:border-orange-500/50 transition-all duration-300"
    >
      ...
      <h4 className="text-white font-bold text-[10px] sm:text-sm md:text-lg leading-tight uppercase tracking-tight">
        {person.name}
      </h4>
      <p className="text-orange-500 font-mono text-[8px] sm:text-[10px] uppercase mt-0.5 sm:mt-1 tracking-wider sm:tracking-widest">
        {person.role}
      </p>
      {person.desc && (
        <p className="text-slate-500 text-[7px] sm:text-[9px] mt-1 sm:mt-2 uppercase leading-tight italic">
          {person.desc}
        </p>
      )}
    </motion.div>
